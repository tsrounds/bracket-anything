    ...
    const fetchQuizAndSubmissions = async () => {
      try {
        ...
        if (quizDoc.exists()) {
          ...
          // Fetch current user's profile for edit functionality
          if (user) {
            try {
              const userProfileDoc = await getDoc(doc(db as any, 'userProfiles', user.uid));
    ...
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center relative">
      ...
      {userName && (
        <button
          ...
          className="absolute top-4 right-4 md:top-8 md:right-8 hover:opacity-80 transition-opacity cursor-pointer z-10"
        >
          <UserInfoBadge
            name={userName}
            avatar={userAvatar}
